...
public class TPSlider : MonoBehaviour {
	...
	void Update () {
		if (slider1Value <= 1.0f)
		{
			...
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup2", 0f));
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup3", 0f));
		}
		else if (slider1Value <= 2.0f)
		{
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup", slider0Value));
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup1", 1f));
		...
		else if (slider1Value <= 3.0f)
		{
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup", slider0Value));
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup1", 1f));
			TP.SetAttribute(new PKFxManager.Attribute("TeleportStartup2", 1f));
